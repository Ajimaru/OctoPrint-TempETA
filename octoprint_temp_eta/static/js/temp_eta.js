...
$(function() {
    ...
    $(document).on("click", "#temp_eta_reset_profile_history", function(e) {
        ...
        var successWithCount = _attrOr($btn, "data-success-text-with-count", "Deleted ({count}).");
        var successNone = _attrOr($btn, "data-success-text-none", "Nothing to delete.");
        var errorTitle = _attrOr($btn, "data-error-title", notifyTitle);
        var errorText = _attrOr($btn, "data-error-text", "Failed.");
        var errorNoApi = _attrOr($btn, "data-error-no-api", "OctoPrint API not available.");
            ...
            OctoPrint.simpleApiCommand("temp_eta", "reset_profile_history", {})
                .done(function(resp) {
                    if (resp && resp.success) {
                        var deletedFiles = (typeof resp.deleted_files === "number") ? resp.deleted_files : null;
    ...
    function TempETAViewModel(parameters) {
        ...
        self._bindElementOnce = function(selector, dataFlag, maxAttempts, delayMs) {
            var attempts = 0;
        ...
        self._ensureSidebarBound = function() {
            ...
            self._bindElementOnce("#sidebar_plugin_temp_eta", "tempEtaKoBoundSidebar", 10, 200);
        };
        ...
        self._isFrontendDebugEnabled = function() {
            ...
            try {
                return typeof ps.debug_logging === "function" ? !!ps.debug_logging() : !!ps.debug_logging;
            } catch (e) {
                return false;
            }
        };
        ...
        self._debugLog = (function() {
            ...
            return function(key, message, payload, minIntervalMs) {
                ...
                var interval = typeof minIntervalMs === "number" ? minIntervalMs : 30000;
        ...
        self._pluginSettings = function() {
            try {
                return self.settings && self.settings.plugins && self.settings.plugins.temp_eta;
            } catch (e) {
                return null;
            }
        };
        ...
        self._effectiveThresholdUnit = function() {
            ...
            var raw = typeof ps.threshold_unit === "function" ? ps.threshold_unit() : ps.threshold_unit;
            if (raw === "octoprint") {
        ...
        self.thresholdStartDisplay = ko.pureComputed({
            read: function() {
                ...
                var thresholdC = parseFloat(typeof ps.threshold_start === "function" ? ps.threshold_start() : ps.threshold_start);
            ...
            write: function(value) {
                var ps = self._pluginSettings();
                if (!ps || !ps.threshold_start || typeof ps.threshold_start !== "function") {
                    return;
                }
                ...
                var thresholdC = self._effectiveThresholdUnit() === "f" ? self._fToC(numeric) : numeric;
        ...
        self.onDataUpdaterPluginMessage = function(plugin, data) {
            ...
            if (data.type === "settings_reset") {
                try {
                    if (self.settingsViewModel && typeof self.settingsViewModel.requestData === "function") {
            ...
            if (data.type === "eta_update") {
                ...
                if (!self.heaterData[heater]) {
                    ...
                    self._debugLog("register_" + heater, "[TempETA] Registered new heater", heater, 60000);
                }
        ...
        self.isHeaterSupported = function(heaterName) {
            ...
            if (!profile && self.printerState && typeof self.printerState.printerProfile === "function") {
                profile = self.printerState.printerProfile();
            } else if (!profile && self.printerState && typeof self.printerState.currentPrinterProfileData === "function") {
            ...
            var hasBed = profile.heatedBed === undefined ? true : toBoolFlag(profile.heatedBed);
        ...
        self.displayHeaters = ko.computed(function() {
            var heaters = self.heaters().filter(function(h) {
                return h.actual() !== null || h.target() !== null || self.isETAVisible(h.eta());
            });
        ...
        self.visibleHeaters = ko.computed(function() {
            return self.displayHeaters().filter(function(h) { return self.isETAVisible(h.eta()); });
        });

        // Navbar visibility (used by template binding)
        self.showETA = ko.pureComputed(function() {
            return self.isComponentEnabled("navbar") && self.visibleHeaters().length > 0;
        });

        self._applyComponentVisibility = function() {
            // Sidebar: hide whole box wrapper if disabled or no ETA to show.
            var showSidebar = self.isComponentEnabled("sidebar") && self.displayHeaters().length > 0;
        ...
        self._setupVisibilitySubscriptions = function() {
            ...
            ["enabled", "show_in_sidebar", "show_in_navbar", "show_in_tab"].forEach(function(key) {
                try {
                    var obs = ps[key];
            ...
            try {
                if (self.visibleHeaters && typeof self.visibleHeaters.subscribe === "function") {
                    self.visibleHeaters.subscribe(self._applyComponentVisibility);
                }
            } catch (e) {}

            try {
                if (self.displayHeaters && typeof self.displayHeaters.subscribe === "function") {
