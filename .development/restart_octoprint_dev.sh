...
verify_plugin_loaded() {
  ...
  echo "--- temp_eta verification (log tail) ---"
  ...
  if ! wait_for_log "Enabled plugin temp_eta|octoprint_temp_eta/__init__|octoprint\\.plugins\\.temp_eta" 15 0.5; then
    echo "temp_eta: not detected in startup log (likely not installed); skipping verification"
    return 0
  fi

  # 1) Confirm the plugin started
  if wait_for_log "octoprint\.plugins\.temp_eta - INFO - Temperature ETA Plugin started" 60 0.5; then
    echo "temp_eta: started OK"
  else
    echo "WARNING: temp_eta start message not found in log (since restart)." >&2
  fi

  # 2) Show the actual metadata path OctoPrint used for temp_eta (scoped to log lines since restart).
  ...
  meta_path="$(log_since_start | awk '
    ...
    END {
      for (i = n; i >= 1; i--) {
        ...
        if (index(combo, "Parsing plugin metadata from AST of") > 0 && index(combo, "octoprint_temp_eta/__init__") > 0) {
          if (match(combo, /Parsing plugin metadata from AST of ([^\r\n]*octoprint_temp_eta\/__init__\.py)/, m)) {
            print m[1]
            exit 0
          }
        }
  ...
  if [[ -n "$meta_path" ]]; then
    echo "temp_eta: metadata path: $meta_path"
  else
    echo "WARNING: Could not find a fresh 'Parsing plugin metadata...' line for temp_eta (since restart)." >&2
  fi

  # 3) Confirm OctoPrint enabled the plugin (i.e., not in safe mode)
  if wait_for_log "Enabled plugin temp_eta" 60 0.5; then
    echo "temp_eta: enabled OK"
  else
    echo "WARNING: Did not find 'Enabled plugin temp_eta' in log (since restart)." >&2
  fi
}
