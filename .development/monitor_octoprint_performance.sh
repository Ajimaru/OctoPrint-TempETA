...
Options:
  ...
  --data-dir PATH        Plugin data dir to monitor (default: basedir/data/temp_eta)
...
PLUGIN_DATA_DIR_DEFAULT="$OCTOPRINT_BASEDIR/data/temp_eta"
...
write_header() {
  echo "timestamp_iso,pid,cpu_percent,mem_percent,rss_kb,vsz_kb,etimes_s,threads,fd_count,octoprint_log_bytes,temp_eta_tail_lines,data_dir_bytes,json_count,json_bytes,tmp_count" >"$CSV_PATH"
}
...
temp_eta_tail_lines() {
  local path="$1"
  if [[ -f "$path" ]]; then
    tail -n 200 "$path" 2>/dev/null | grep -ci 'temp_eta' || echo 0
  else
    echo 0
  fi
}
...
sample_once() {
  ...
  local tail_lines
  tail_lines="$(temp_eta_tail_lines "$OCTOPRINT_LOG_PATH")"
